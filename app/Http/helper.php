...
if (!function_exists('get_parent')) {
	function get_parent($dep_id) {
		...
		$department = \App\Departments::find($dep_id);
		if ($department->parent !== null && $department->parent > 0) {
			// array_push($lest_department, $department->parent);
			return get_parent($department->parent).','.$dep_id;
		} else {
			return $dep_id;
		}

		// return $lest_department; 
	}
}
...
if (!function_exists('check_mall')) {
 	function check_mall($id, $pid) {
 		return \App\Model\MallProduct::where('product_id', $pid)
 		 							->where('mall_id', $id)->count()>0?true:false;
 	}
 } 
...
if (!function_exists('load_dep')) {
	function load_dep($select = null, $dep_hide = null) {
		...
		$departments = \App\Departments::selectRaw('name as text')
		->selectRaw('id as id')
